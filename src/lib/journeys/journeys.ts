...
export const JOURNEYS: Journey[] = [
  ...
  // ============================ Create an SOP ============================
  {
    ...
    summary: 'An admin signs in and lands directly on the merged SOP surface (UX-01 one home per role), with an Admin scope group of lenses alongside the worker scopes. Admin surfaces live in the primary header for admin roles — Governance · Create New SOP · Team · Settings sit right of Sign-off (2026-07-30, supersedes the UX-02 account-menu door).',
    steps: [
      { id: 's', type: 'start', label: 'Signed in as admin / safety manager' },
      { id: 'home', type: 'screen', label: 'Admin home — SOP surface', route: '/sops', detail: 'roleHome(admin) lands here via the brand mark. The primary header adds Governance (/sops?view=attention) · Create New SOP (/admin/sops/new) · Team · Settings for admin roles; worker surfaces (SOPs · Sign-off) stay one tap away.' },
      { id: 'menu', type: 'decision', label: 'Open another admin surface? (app header: Governance · Create New SOP · Content · Team · Settings; Needs attention + Access are lenses within the Admin scope group on the SOP surface)', branches: [
        { label: 'SOPs', to: 'sops' },
      ...
      { id: 'sops', type: 'screen', label: 'SOP surface (Admin scope group)', route: '/sops', detail: 'The "Needs attention" scope (governance queue, grouped by worst flag) deep-links /sops?view=attention — the folded needs-attention lens (UX-03): governance queue, filter chips, header flag counts. An "Access" scope deep-links /sops?view=access — the D-hybrid wiring surface (D-09), another lens on this same surface.' },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'legacy', type: 'screen', label: 'Legacy governance URL (optional)', route: '/admin/governance', detail: 'Redirect shim → /sops?view=attention — legacy ?filter=X deep-links map onto the folded view’s filter param (GQ-04 bookmarks keep working).' },
      { id: 'legacy-admin-sops', type: 'screen', label: 'Legacy /admin/sops URL (optional)', route: '/admin/sops', detail: 'Redirect shim → /sops, preserving ?view=/?status=/?owner=/?filter=/?departments=/?collection=/?sop= so old bookmarks keep working (SUR-01).' },
      { id: 'queue', type: 'screen', label: 'Needs attention view', route: '/sops?view=attention', detail: 'The governance queue folded into the SOP surface (?view=attention). Header chips count each flag with deep-links; one list, filter chips, ONE primary action per row. Computed on read — no jobs, no materialized state (D28-05).' },
