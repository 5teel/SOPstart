...
export const JOURNEYS: Journey[] = [
  ...
  // =========================== Refine & publish ===========================
  {
    ...
    steps: [
      ...
      { id: 'publish', type: 'action', label: 'Publish', route: '/admin/sops/[sopId]/publish', detail: 'SOP goes live for assigned workers. A "Wire up access →" CTA appears once published (D-12a).' },
  ...
  },
  {
    ...
    group: 'Refine & publish',
    persona: 'SOP Admin',
    title: 'Wire up library access',
    summary: 'After publishing, an admin grants which org units (and people) can see the SOP in their library by visually wiring org units to library collections — the same D-hybrid surface also works as a library filter (SC-4) and reaches any unwired or existing SOP organically (D-12b).',
    steps: [
      { id: 's', type: 'start', label: 'SOP just published (or any unwired SOP)' },
      { id: 'cta', type: 'action', label: '"Wire up access →" CTA on the Publish stage', detail: 'D-12a — only shown once the SOP is published; jumps straight into connect mode for it.' },
      { id: 'access', type: 'screen', label: 'Access view', route: '/admin/sops?view=access', detail: 'D-hybrid wiring surface (D-09): org units (org/area/department/person) on the left, library collections on the right, quiet-by-default trace-on-click. The CTA pins this SOP NEW · UNWIRED atop the library column; opening the view directly (D-12b) reaches any SOP the same way.' },
      { id: 'connect', type: 'action', label: 'Toggle org units', detail: 'Each toggle draws a live wire and updates a "Visible to N people via M grants" blast-radius banner.' },
      { id: 'done', type: 'action', label: '✓ Done wiring', detail: 'Writes additive grants (D-11) via createGrant, materializing into sop_departments/sop_access_people.' },
      ...
      { id: 'e', type: 'end', label: 'Access wired' },
    ],
  },
  {
    id: 'assign-sop',
