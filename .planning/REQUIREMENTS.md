...
- [x] **OWN-02**: Admin can reassign an SOP's owner in ≤2 clicks from the SOP itself (library row or builder/detail), not a buried settings page
...
- [x] **OWN-04**: An owner sees a "My SOPs" view listing the SOPs they own with review status at a glance
...
- [x] **REV-02**: Overdue SOPs show a visible "review overdue" state in the admin library (grey-out/badge) — worker read/walkthrough access is NEVER blocked by review state
- [x] **REV-03**: Workers see a lightweight currency indicator ("current as of <date>") on the SOP view — informational only, zero interaction cost
...
- [x] **GQ-02**: Every queue row has a one-click primary action inline (confirm current / assign owner / open in builder / fix assignment)
...
- [x] **GQ-04**: Governance dashboard widget on the admin home shows queue counts (overdue / unowned / awaiting approval) with deep links into the queue

### Approval Chains (APR)

- [x] **APR-01**: Admin can define an optional approval chain (1–4 ordered approver steps, by role or named member) per SOP category; categories without a chain publish exactly as today
...
- [x] **UX-03**: One governance surface — queue folds into /admin/sops as the "Needs attention" view with flag chips; GovernanceWidget + LibraryReviewCell removed; approval-chain editor relocates to admin settings; APR-03/04 approve-from-queue preserved verbatim
...
- [x] **UX-06**: Admin SOP list rows are one line (title · status chip · flag chip · owner); per-SOP actions live in the builder as a labelled action menu
...
| REQ-ID | Phase | Status |
|--------|-------|--------|
| OWN-01..04 (4) | Phase 28 | Complete |
| REV-01..04 (4) | Phase 28 | Complete |
| REV-05 (1) | Phase 31 | Pending |
| GQ-01..04 (4) | Phase 28 | Complete |
| APR-01..05 (5) | Phase 29 | Complete |
| UX-01..08 (8) | Phase 30 | Pending |
| TRN-01..03 (3) | Phase 31 | Pending |

**v6.0 Coverage:**

- v6.0 requirements: 29 total (OWN ×4, REV ×5, GQ ×4, APR ×5, UX ×8, TRN ×3)
- Mapped to phases: 29 (Phase 28: OWN-01..04, REV-01..04, GQ-01..04 = 12; Phase 29: APR-01..05 = 5; Phase 30: UX-01..08 = 8 — inserted 2026-07-12 from frontend audit; Phase 31: TRN-01..03, REV-05 = 4)
- Unmapped: 0
...
- [x] **SUR-03**: One top-level "SOPs" entry across all roles — no second identically-labelled door, and no role sees two nav paths to the same list
