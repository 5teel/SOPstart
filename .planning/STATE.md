...
stopped_at: Completed 26-10-PLAN.md (R4 — smart-next ghosts: confident non-redundant predictions from the shared SMART map render as inline dashed-purple --ai ghosts; useSmartGhosts pure state machine (computeGhosts/resolveGhostVisibility/ghostsGoneOnTyping) + rAF/ref hook drives the 5-scenario sketch matrix — appear/self-suppress/one-live-dim/scroll-past-gone/typing-dismiss; Tab-or-click accept → content-ops insertBlock → autosave; NO React state on the scroll path, document never re-renders on scroll; GhostRow wired into EditableDocument between blocks; ghosts.spec 9 green, tsc + build green)
