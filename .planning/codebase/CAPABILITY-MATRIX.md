...
| Capability | worker | supervisor | admin | safety_manager | SOP owner (any role) | Enforced at |
|---|---|---|---|---|---|---|
...
| Edit SOP content | — | — | ✅ | ✅ | ✅ (own SOP only) | `requireSopEditAccess()` in `src/lib/auth/guards.ts` (Phase 46 CAP-02) — admin/safety_manager unconditionally, or caller is `sops.owner_user_id`; RLS `admins_can_manage_sections`/`_steps`/`_images` carry the same OR-condition as backstop |
| Verify blocks | — | — | ✅ | ✅ | — | Per-block verify checklist gate, `requireAdminContext()`-gated — **not** extended to owners by CAP-02 (scoped to content-edit only, RESEARCH § SOP Edit Path Inventory Scope decision) |
| Publish SOP | — | — | ✅ | ✅ | — | `assertPublishGates()` via `requireAdmin()` in `src/lib/governance/publish-core.ts` — owner does not gain publish rights |
| Delete SOP | — | — | ✅ | ✅ | — | `requireAdminContext()` — owner does not gain delete rights |
| Version history | — | — | ✅ | ✅ | — | `cloneSopAsDraft`/`restoreVersionAsNew`/`uploadNewVersion` in `src/actions/versioning.ts`, manual role check + service-role self-enforced org-scope — owner does not gain version-supersede rights |
...
**Mapping in force:** sign-off authority = `sops.owner_user_id` (Phase 28 single accountable owner — every SOP has exactly one owner, backfilled, reassignable by an admin via `setSopOwner`/`OwnerPicker`). An owner who is not admin/safety_manager gets content edit rights (sections, steps, images, layout_data, block junctions) on that SOP only, via `requireSopEditAccess()`. Publish, verify-blocks, delete-SOP, version-supersede, and owner-reassignment stay admin/safety_manager-only — the rule is scoped to "edit," not to admin powers generally (RESEARCH § SOP Edit Path Inventory Scope decision, A2).

**A1 — open confirmation for Simon:** this mapping is MEDIUM confidence, not settled fact. If "sign-off authority" was actually meant to reference **approval-chain approvers** (Phase 29 `approval_chains`/`sop_approvals` — category-scoped, 1-4 ordered steps, optional per category) rather than the single SOP owner, the predicate changes shape: from "one owner per SOP" to "any named approver in that SOP's category chain," and some SOPs (those without a configured chain) would have zero people with sign-off-derived edit rights instead of exactly one. The code change to flip this is a single predicate swap in `requireSopEditAccess()` — cheap if caught now, more disruptive if assumed settled and shipped wrong.
...
| SOP owner edit guard (CAP-02) | `requireSopEditAccess()`, `src/lib/auth/guards.ts` (added Phase 46 plan 03) |
...
3. **`sop_section_blocks` RLS is `using(true)`** by design (00030/00031 recursion-avoidance pattern — a junction table referencing `sops` in its own RLS policy recurses). The app-level guard in `src/actions/sop-section-blocks.ts` (`requireAdmin()` → `requireAdminContext()`, swapped to `requireSopEditAccess()` in 46-03) is the **only** enforcement for block-junction writes; RLS provides no backstop here. This is intentional, not a gap to close — flagged so a future reader does not assume RLS covers this table the way it covers `sop_sections`/`sop_steps`/`sop_images`.
