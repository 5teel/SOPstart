...
| Edit SOP content | — | — | ✅ | ✅ | ✅ (own SOP only) | `requireSopEditAccess()` in `src/lib/auth/guards.ts` (Phase 46 CAP-02) — admin/safety_manager unconditionally, or caller is `sops.owner_user_id`; RLS `admins_can_manage_sections`/`_steps`/`_images` (00063) and `ssb_admin_manage_own_org` on block junctions (00064, full predicate in USING and WITH CHECK) carry the same OR-condition as backstop |
...
3. **`sop_section_blocks` block-junction writes** — CLOSED by the Phase 46 review-fix pass (CR-02, migration `00064_ssb_owner_edit.sql`). The app-level guard (`requireSopEditAccess()` in `src/actions/sop-section-blocks.ts`, 46-03) is now backstopped by RLS: `ssb_admin_manage_own_org` carries the owner arm nested inside the org-scope AND, with the full predicate restated in **both** USING and WITH CHECK (the 00062 partial-check trap deliberately avoided). Proven live by the junction probes in `tests/phase46/sop-edit-owner-access.spec.ts`. Recorded here because this document originally claimed the junction had no RLS backstop by design — that was true when written and is no longer true.
