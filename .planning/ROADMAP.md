...
- [x] 30-06-PLAN.md — UX-05 worker 6->3 tabs (Read/Walk/Flow) + delete Model/Walkthrough tabs + walkthrough route + dept filter + bundle re-baseline (UX-05, UX-08, UX-04)

**Wave 7**

- [x] 30-07-PLAN.md — UX-06 builder labelled action menu + UX-07 plain language (Check/Edit/Send-to-workers, plain flag titles, offline pill) (UX-06, UX-07)
...
- [x] 32-02-PLAN.md — Wave 1: migrations 00046 schema (7 tables + area_id + grant_subject_type + D-13 RLS arm) + 00047 data seed (day-one equivalence)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 32-03-PLAN.md — Wave 2: [BLOCKING] db push + day-one equivalence assertion + person-grant arm verify
...
- [x] 32-05-PLAN.md — Wave 4: grants.ts (CRUD + materialize into sop_departments + sop_access_people) + org-isolation/person-grant runtime tests
- [x] 32-06-PLAN.md — Wave 4: auto-layout.ts + OrgChartCanvas + ViewToggle (Node Chart)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 32-07-PLAN.md — Wave 5: OrgColumnsBoard + /admin/team rewrite (D-08) + journeys/uat
- [x] 32-08-PLAN.md — Wave 5: WiringPatchBay + SelectionStrip (D-hybrid grouped/focus/trace + wire-up blast-radius)
...
- [x] 33-03-PLAN.md — [BLOCKING] live `db push` 00050 + pre/post grant-row equivalence + index-swap assertions
- [x] 33-04-PLAN.md — Wayfinder builder header (light bar, inline lock reason) + ONE "Tools for this SOP" menu (SC-6)

**Wave 3**

- [x] 33-05-PLAN.md — grants.ts SOP-target arm + narrowing-override materialization + LIVE ephemeral-org runtime tests (SC-3/SC-4)
- [x] 33-06-PLAN.md — WiringPatchBay full teams ladder: dept→role→person tiers, dashed vacancies (SC-1)

**Wave 4**

- [x] 33-07-PLAN.md — dual-write closure: assignSopDepartments/wizard/ai-prompt rewired through SOP-target grants (closes the 32-VERIFICATION anti-pattern)
- [x] 33-08-PLAN.md — collection→SOP drill-down + organic choose-mode + rightEndpoint + chosen-by-name pill (SC-2)
...
- [x] 33-10-PLAN.md — CR-01: deleteSop org-ownership guard (cross-tenant delete blocked) + live/source-contract test (SC-6)
- [x] 33-11-PLAN.md — WR-02: migration 00051 all_departments_pre_override snapshot/restore in materializeSopAccessForOrg + live re-follow test (SC-4)
...
- [x] 34-01-PLAN.md — phase34 Playwright project + 5 Wave-0 stub specs (OBS-01/02/03 + SC-4 cross-org isolation)
- [x] 34-02-PLAN.md — migration 00052 sop_observations (append-only RLS + worker self-read + verdict check) + organisations.observation_labels (D-02) + RecordObservationSchema validator (files only)

**Wave 2** *(blocked on Wave 1)*

- [x] 34-03-PLAN.md — [BLOCKING] db push 00052 + PGRST reload + live cross-org isolation (SC-4) + append-only immutability runtime proofs

**Wave 3** *(blocked on Wave 2)*

- [x] 34-04-PLAN.md — observations.ts server actions (recordObservation session-client/no-admin-client + version auto-stamp + listObservationsForWorker/Person + listWorkerSopsForPicker + get/setObservationLabels) + record/version specs live

**Wave 4** *(blocked on Wave 3)*

- [x] 34-05-PLAN.md — shared RecordObservationModal + VerdictButtons + ObservationRow (org labels, declared tokens, permanent-record framing)

**Wave 5** *(blocked on Wave 4)*

- [x] 34-06-PLAN.md — entry A: PersonPanel + org-chart/columns person-click via TeamViewShell + verdict-label settings editor (D-02, D-03)
- [x] 34-07-PLAN.md — entry B: /activity header button + CompletionSummaryCard "I observed this" row action (D-03, D-11)
- [x] 34-08-PLAN.md — worker /profile "Observations about you" + trust banner (OBS-02, D-08) + worker-visibility spec live + journeys/uat

**Wave 6** *(blocked on Wave 5)*

- [x] 34-09-PLAN.md — merged-tree gate: full suite + tsc + next build + pathways coverage + OBS-01/02/03 + SC-4 audit
...
- [x] 35-01-PLAN.md — Pure competency core (classify/matrix/csv) + Wave-0 harness (phase35 + phase35-unit projects, CMP-04 guard, MTX-02 no-double-derivation contract)

**Wave 2** *(blocked on Wave 1)*

- [x] 35-02-PLAN.md — Server actions (matrix/record/self/CSV) + Zod filter validators + StatePill/CSS tokens + auth/org-scope source-contract + per-role RLS probe
...
- [x] 36-01-PLAN.md — phase36 Playwright project + 3 spec files (live CMP-04-style informational-only guard + CMP-03/TRN-03 self-activating stubs)
- [x] 36-02-PLAN.md — migration 00055 `sops.refresher_interval_months` + database.types.ts + pure `isOutdatedVersion` / `refresherDueDate` modules + unit tests

**Wave 2** *(blocked on Wave 1)*

- [x] 36-03-PLAN.md — [BLOCKING] db push 00055 + PGRST reload + live column/constraint proof; interval copy-forward through BOTH supersede paths; `setRefresherInterval` admin action
- [x] 36-04-PLAN.md — pure layer: MatrixCell/rollup version-currency + refresher fields, CSV `on_current_version` / `refresher_due_date`, unit coverage incl. never-demote regression

**Wave 3** *(blocked on Wave 2)*

- [x] 36-05-PLAN.md — lineage-widen `getTrainingMatrix` / `getTrainingRecordForPerson` / `getMyCompetencyStates` (closes the CMP-03 evidence-orphaning gap) + org-scope guard spec

**Wave 4** *(blocked on Wave 3)*

- [x] 36-06-PLAN.md — populate the two CSV columns from lineage data + `getVersionCompletionBreakdown` (TRN-03 read, admin/safety_manager gate)

**Wave 5** *(blocked on Wave 4)*

- [x] 36-07-PLAN.md — StatePill outdated/refresher sibling chips + matrix rollup tallies + axis-swap toggle (folded todo)
- [x] 36-08-PLAN.md — worker library refresher chip (SopLibraryCard + Your SOPs computation) + bundle gate
- [x] 36-09-PLAN.md — versions page: TRN-03 per-version completion breakdown panel + per-SOP refresher interval control

**Wave 6** *(blocked on Wave 5)*

- [x] 36-10-PLAN.md — live supersede/orphaning runtime probe, guard extended to all surfaces, journeys.ts + uat/tests.ts, full phase gate
...
- [x] 37-01-PLAN.md — phase37 Playwright project + 6 Wave-0 specs (CMP-04 worker guard live) + migration 00056 file (override audit columns, CHECK constraints, insert-policy override clause, worker_notifications.subject_user_id) + database.types.ts + `overrideReason` on both write schemas
- [x] 37-02-PLAN.md — pure `isSignedOffAssessor` predicate (classifyCompetency + resolveLineage, no forked ladder, D-01/D-02) + 8 behavioural unit tests

**Wave 2** *(blocked on Wave 1)*

- [x] 37-03-PLAN.md — [BLOCKING] db push 00056 + live column/constraint/policy proof; `recordObservation` gate on `performed_to_sop` only (D-03/D-04); `getAssessorStatusForSop` / `requestAssessorReview` / `listAssessmentRequests` (D-08)

**Wave 3** *(blocked on Wave 2)*

- [x] 37-04-PLAN.md — `signOffCompletion` gate on `approved` only + role array widened to admin (Pitfall 2, D-06) + blocked/override sign-off UI with request CTA
- [x] 37-05-PLAN.md — three-state recording modal (blocked teaching copy, override disclosure, coaching untouched — D-08/D-09/D-05) + AssessmentRequestsPanel on /admin/team
...
- [x] 40-06-PLAN.md — DAT-01c: ordered applier, three-pass null-clobber-safe backfill + settings remap, and the [BLOCKING] live production proof (SC-5)
- [x] 40-09-PLAN.md — DUP-04: one `AdminPageShell` with a contextual back-link slot across all five non-compliant admin creation routes
...
- [x] 41-03-PLAN.md — Wave 1: `listAdminAccessData` action + `AdminAccessLens` wrapping the wiring patch bay
...
- [x] 41-05-PLAN.md — Wave 3: the merged `/sops` surface — Admin scope group, three `dynamic({ ssr:false })` lenses, deep-link resolution, `replaceState` scope state, capability matrix
...
- [x] 41-08-PLAN.md — Wave 5: repoint the nine legacy source-contract specs to the code's new homes; full suite green
